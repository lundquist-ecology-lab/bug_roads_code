...
def analyze_euclidean_distances():
    logging.info("Loading path results...")
    paths_gdf = gpd.read_file("../outputs/results.gpkg", layer="results_point_id_fixed")

    logging.info("Filtering Euclidean paths to <= 1000m...")
    paths_gdf = paths_gdf[paths_gdf["euclidean_distance"] <= 1000]
    logging.info(f"Number of Euclidean paths <= 1000m: {len(paths_gdf)}")
    ...
    output_gpkg = "../results/filtered_euclidean_paths_with_boroughs.gpkg"
    ...
    stats_path = output_dir / "euclidean_distance_analysis_1000m.txt"
    with open(stats_path, "w", encoding="utf-8") as f:
        f.write(f"Euclidean Distance Analysis Results (Distances <= 1000m)\n")
        f.write(f"Generated on: {timestamp}\n")
        f.write("=" * 50 + "\n\n")

        logging.info("Calculating citywide statistics...")
        citywide_stats = get_summary_stats(paths_gdf["euclidean_distance"])
        ...
        for boro in boroughs_list:
            ...
            if len(boro_data) > 0:
                f.write(format_stats(get_summary_stats(boro_data["euclidean_distance"]), boro))
        ...
        for i, b1 in enumerate(boroughs_list):
            for b2 in boroughs_list[i + 1:]:
                ...
                if len(boro_pair) > 0:
                    f.write(format_stats(get_summary_stats(boro_pair["euclidean_distance"]), f"{b1} <-> {b2}"))

    logging.info(f"Analysis completed successfully. Results written to {stats_path}")

if __name__ == "__main__":
    analyze_euclidean_distances()
