...
class PathFinder:
    ...
    @staticmethod
    def find_nearest_network_point(geometry, kd_tree, coords):
        """Find the nearest point in the network to a given geometry"""
        # Get a representative point from the geometry (centroid or a point on the surface)
    ...
    def process_single_pair(self, pair, kd_tree, coords, graph):
        ...
        try:
            # Find nearest network points using centroids
            start_point = self.find_nearest_network_point(pair['geometry1'], kd_tree, coords)
            end_point = self.find_nearest_network_point(pair['geometry2'], kd_tree, coords)
...
if __name__ == "__main__":
    ...
    pathfinder.run(start_batch=403, num_processes=6)
