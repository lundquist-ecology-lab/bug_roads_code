...
centroids_df = centroids_df[['fid', 'name311', 'acres']]
centroids_df['area_m2'] = centroids_df['acres'] * 4046.86  # Convert acres to m²

# Merge the parks data with the centroids data to get area_m2 and name311
merged_df = pd.merge(parks_df, centroids_df, on='name311')

# Create a dictionary to map fids to name311
fid_to_name311 = centroids_df.set_index('fid')['name311'].to_dict()
...
def calculate_cluster_coverage(cluster_file, radius, veg_type):
    ...
    cluster_df['fid_list'] = cluster_df['fid'].apply(lambda x: list(map(float, x.split(', '))))
    
    # Calculate total vegetation cover for each cluster
    total_coverage = []
    for index, row in cluster_df.iterrows():
        fids = row['fid_list']
        name311s = [fid_to_name311.get(fid) for fid in fids]
        total_cover = merged_df[merged_df['name311'].isin(name311s)][veg_type].sum()
...
cluster_files = [
    ('/neighborhoods/euclidian/nyc_clusters_at_100.csv', 100),
    ('/neighborhoods/euclidian/nyc_clusters_at_250.csv', 250),
    ('/neighborhoods/euclidian/nyc_clusters_at_500.csv', 500),
    ('/neighborhoods/euclidian/nyc_clusters_at_1000.csv', 1000)
]
...
for veg_type in vegetation_types:
    ...
    for cluster_file, radius in cluster_files:
        ...
        print(f"  Total clusters: {results['Total Parks']}")
